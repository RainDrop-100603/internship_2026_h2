# BTP Trial + Eclipse/ADT 설치 가이드

> 실제 진행하면서 겪은 선택지·트러블슈팅을 순서대로 기록. 공유용.
> 진행 기준: Windows 11, 한국, 개인 교육용 계정 (2026-07)
>
> 본문은 **핵심 절차** 위주. 각 단계에서 막히면 바로 아래 접힌 `트러블슈팅` 항목을 펼쳐볼 것. 참고성 상세 자료는 [하단 부록](#부록), 환경·도구 선택의 배경은 [research-notes.md](research-notes.md) 참고.
...
<details>
<summary>트러블슈팅: 전화번호 인증 (SMS)</summary>
...
- [Eclipse 다운로드 페이지](https://www.eclipse.org/downloads/)에서 **Eclipse IDE for Java Developers** 다운로드 (Enterprise Java, C/C++ 등은 불필요)
- 아키텍처: 일반 Intel/AMD PC는 **x86_64** (PowerShell에서 `echo $env:PROCESSOR_ARCHITECTURE` → `AMD64`면 x86_64. AArch64는 ARM 칩용)
- JRE/JDK 별도 설치 불필요 — 최신 Eclipse 패키지에 Temurin JRE가 번들되어 있음
- **설치 폴더·워크스페이스는 영문 경로로 지정** (사용자명이 한글이면 필수):
  ```
  C:\eclipse\java-2026-06     ← Eclipse 설치 폴더
  C:\workspace                ← Eclipse 워크스페이스 (프로젝트 저장 위치)
  ```
- zip 압축 해제(또는 설치기) 후 `eclipse.exe` 실행 → 워크스페이스 지정

<details>
<summary>상세: 왜 영문 경로인가 / 워크스페이스를 git 저장소 밖에 두는 이유</summary>
...
- `C:\` 루트 하위 직접 지정 이유: 한글 경로 이슈 회피 + 경로 길이 제한(MAX_PATH) 이슈도 줄어듦
- **워크스페이스는 git 저장소 하위에 두지 말 것** (로컬 `C:\workspace` 사용):
  ...
  - ABAP 소스 백업/버전관리는 abapGit으로 별도 진행하는 게 정석 (아래 8번)
  - 워크스페이스의 `.metadata`(IDE 설정·인덱스·로그)가 git에 딸려 들어가면 저장소만 지저분해짐

</details>

<details>
<summary>트러블슈팅: eclipse-inst-jre-win64.exe 실행해도 아무 반응 없음 (한글 사용자명)</summary>
...
<details>
<summary>참고: 실행 시 나타나는 보안 관련 창 (Microsoft Defender Exclusion Check)</summary>
...
<details>
<summary>참고: Trust Artifacts의 Unsigned 항목 경고</summary>
...
- `File > New > ABAP Class` → Package: 본인 패키지(아래 공유 시스템 주의 참고), Name: `ZCL_HELLO_<고유접미사>`
- 코드:
  ```abap
...
<details>
<summary>트러블슈팅: Project Explorer가 안 보임</summary>

- `Window > Show View > Project Explorer` (또는 `Window > Perspective > Reset Perspective`)
...
- ABAP Cloud에서는 클래식 ABAP(Function Module, 리포트, SAP GUI)을 만들 수 없음 → 상세는 [부록 A. ABAP Cloud 언어 제약](#부록-a-abap-cloud-언어-제약--만들-수-있는-것--없는-것)

## 8. abapGit 세팅 (90일 만료 대비 백업 + 코드 공유)

> BTP ABAP 환경은 서버 쪽 abapGit이 **이미 내장** — 온프레미스처럼 ZABAPGIT 설치 불필요. Eclipse 플러그인만 추가하면 됨.
> 90일 만료 정책 상세: [research-notes.md 조사 3](research-notes.md#조사-3-btp-trial-90일-정책)
...
<details>
<summary>트러블슈팅 / 참고: No ABAP Project Active, 보안 저장소, 복원, 3인 공유</summary>
...
> 서버 선정 배경(fr0ster/mcp-abap-adt 채택, vibing-steampunk 보류 이유): [research-notes.md 조사 4](research-notes.md#조사-4-claude-code--btp-trial-abap-연동-mcp--2026-07)
...
5. **프로젝트 루트에 `.mcp.json` 등록** (비밀정보 없음 — env 파일 경로만 참조하므로 커밋 안전):
   ...
   ```
   - `--exposition`: 노출할 도구 세트 선택. ⚠️ **`high`와 `low`는 동시 지정 불가** — 옵션 상세는 [부록 B](#부록-b-mcp-abap-adt-설정-레퍼런스-v881-기준)
6. **Claude Code 재시작** → `abap-trial` MCP 로드됨

<details>
<summary>트러블슈팅: MCP가 조용히 로드 안 됨 — exposition에 high,low 동시 지정</summary>

- **증상**: Claude Code 세션에서 `abap-trial` 도구가 끝내 나타나지 않음. 에러 표시도 없어 접속 정보 문제로 오인하기 쉬움
- **진단**: 서버를 터미널에서 직접 실행해보면 즉시 원인 출력:
  ```
  mcp-abap-adt --transport=stdio --env-path=...\TRIAL.env --exposition=readonly,high,low
  → [MCP] launcher failed: Invalid exposition: 'high' and 'low' are mutually exclusive
  ```
- **원인**: 초기 세팅 때(구버전)는 `readonly,high,low`가 유효했으나, **v8.8.x에서 `high`/`low` 상호 배타 검증이 추가**되어 서버가 기동 자체에 실패
- **해결**: `.mcp.json`에서 `--exposition=readonly,high`로 수정 (2026-07-19 반영). 저수준 작업이 필요해지면 그때 `readonly,low`로 임시 전환
- **교훈**: MCP 서버가 안 붙으면 `.mcp.json`의 command를 터미널에서 그대로 실행해 stderr를 볼 것 — 하네스는 기동 실패를 조용히 삼킴
...
- GetClass로 `ZCL_HELLO_WORLD_PKS` 소스 코드 조회 정상 (JWT 인증 동작 확인)
- `readonly,high` 기준 노출 툴 약 215개: 테이블 데이터 조회(GetTableContents), 클래스/CDS/RAP 아티팩트 CRUD, SQL 쿼리(GetSqlQuery), 클래스 실행(RuntimeRunClass), 유닛 테스트(RunUnitTest), Where-Used, 버전 이력/diff, 덤프·프로파일러(Runtime*) 등

### 참고

- **토큰 갱신**: JWT 만료 시 refresh token으로 자동 갱신. refresh token까지 만료되면 세팅 절차 4번 명령 재실행
...
- 상세 학습 자료: [1_사전공부/README.md](../1_사전공부/README.md) 참고

---

## 부록

### 부록 A. ABAP Cloud 언어 제약 — 만들 수 있는 것 / 없는 것
...
### 부록 B. mcp-abap-adt 설정 레퍼런스 (v8.8.1 기준)

**`--exposition` 핸들러 세트** (쉼표로 조합, 기본값 `readonly,high`):

| 세트 | 노출 도구 | 용도 |
|---|---|---|
| `readonly` | `Get*`, `Check*`, `Validate*`, `Lock*`, `Unlock*` + `SearchObject` + 시스템 분석(`GetWhereUsed`, `GetTypeInfo`, `GetAbapAST` 등) | 조회·검증·잠금. `search`, `system` 세트 자동 포함 |
| `high` | `Create*`, `Update*` 등 ADT 표준 경로의 생성/수정 | 안전한 쓰기 작업 (ADT 잠금·검증 경유) |
| `low` | `Update*Low` 등 저수준 직접 조작 | 위험 작업. **`high`와 상호 배타** |
| `compact` | `HandlerCreate/Get/Update/Delete` 4개 | object_type 인자로 라우팅하는 파사드 — 도구 수를 줄이고 싶을 때 |

- **`readonly,high` 실측** (v8.8.1, 2026-07-19): 도구 약 215개 노출. help 문서에는 `Delete*`/`Activate*`가 `low` 소속으로 적혀 있으나, **실제로는 `readonly,high`에서도 노출됨** (ADT 표준 경로의 삭제/활성화는 high로 취급되는 듯). `low`는 ADT를 거치지 않는 저수준 직접 갱신용
- 조회 전용으로 좁히려면 `--exposition=readonly`

**그 외 주요 옵션**:

| 옵션 | 값 | 비고 |
|---|---|---|
| `--transport` | `stdio`(기본)/`http`/`sse` | Claude Code 로컬 연동은 stdio |
| `--env-path` | .env 파일 절대경로 | `--env=TRIAL`로 플랫폼 기본 sessions 폴더에서 찾게 할 수도 있음 |
| `--system-type` | `cloud`(기본)/`onprem`/`legacy` | onprem이면 Programs(리포트) 등 클래식 도구 활성화. BTP는 cloud 그대로 |
| `--connection-type` | `http`(기본)/`rfc` | rfc는 NW RFC SDK + 추가 패키지 필요. BTP는 http |

**.env 인증 변수** (`sap-abap-auth`가 자동 생성 — 직접 편집할 일은 거의 없음): `SAP_URL`, `SAP_AUTH_TYPE=jwt`, `SAP_JWT_TOKEN`, `SAP_REFRESH_TOKEN`, `SAP_UAA_URL`, `SAP_UAA_CLIENT_ID`, `SAP_UAA_CLIENT_SECRET`. 온프레미스는 basic 인증(`SAP_USERNAME`/`SAP_PASSWORD`/`SAP_CLIENT`)
